...
export class KunjunganController {
  ...
  @UseInterceptors(
    FileInterceptor('file', {
      storage: diskStorage({
        destination: 'C:\Foto Test Sikunang',
  ...
  async uploadFile(@UploadedFile() file: Express.Multer.File) {
    ...
    const outputPath = `C:\Foto Test Sikunang/${compressedFilename}`;
    ...
    try {
      ...
      await sharp(inputPath)
        ...
        .jpeg({ quality: 75 }) // Simpan dalam format JPEG dengan kualitas 75%
        .toFile(outputPath);
  ...
  @Get('kunjunganSemuaBulananKaryawan')
  async KunjunganBulananKaryawan() {
    return this.laporanKunjunganService.totalKunjunganKaryawanSemuaBulan();
  }
